...
class ObjectMag:
    ...
    def __init__(self):
        ...
        _base_page_url = 'http://localhost:5000/Page'
        _base_widget_url = 'http://localhost:5000/Widget'
        self.__interface = {
            'usr_get_url': '%s/usr_get_url' % _base_page_url,
            'usr_get_def': '%s/usr_get_def' % _base_widget_url,
            'usr_get_det': '%s/usr_get_det' % _base_widget_url,
            'usr_search': '%s/usr_search' % _base_page_url
        }

        self.__brs = "FIREFOX"  # 浏览器
        self.__env = "TEST"  # 环境

        self.__window = None  # 当前窗口
        self.__root = None  # 佷节点
    ...
    def open_page(self, p_page_id):
        ...
        _url = orc_invoke(self.__interface["usr_get_url"], _para)
        ...
        if self.__root is None:
            ...
            try:
                ...
                elif "FIREFOX" == self.__brs:
                    self.__root = webdriver.Firefox()
                elif "CHROME" == self.__brs:
